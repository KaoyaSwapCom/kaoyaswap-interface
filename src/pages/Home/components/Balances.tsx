...
const Balances: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchTotalSupply() {
      const [
        ...
        getSushiSupply(yam),
        getBalance(library.provider as provider, getSushiAddress(yam), `${contractAddresses.treasuryMasterChef[chainId]}`).then(res => new BigNumber(res)),
        ...
        getBalance(library.provider as provider, getSushiAddress(yam), `${contractAddresses.treasuryTeam[chainId]}`).then(res => new BigNumber(res))
      ]);
      setTotalSupply(supply);
      setStakedBalance(new BigNumber(stakedBalance));
  ...
  const circulatingPercent = totalSupply
    ? `(${((totalSupply.minus(stakedBalance).minus(fishFeedBalance)).div(totalSupply)).times(100).toFixed(2)}%)` : '';

  return (
    <>
      <TotalSupply>
        {account && <div>
          Total Kaoya Supply: {(totalSupply ? getBalanceNumber(totalSupply) : 1100000).toLocaleString('currency', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
        ...
        {account && <div>
         ...
         <a href={`${exploreURL}/address/${contractAddresses.treasuryTeam[chainId]}#tokentxns`} target="_blank"> Treasury Record</a>
        </div>}
      </TotalSupply>
      <StyledWrapper>
        <StyledCard>
            ...
            <StyledBalances>
              <StyledBalance>
                <div style={{ flex: 1 }}>
                  ...
                  <CustomValue>{!!account ? getBalanceNumber(sushiBalance).toFixed(5) : 'Locked'}</CustomValue>
                </div>
              </StyledBalance>
            </StyledBalances>
            <Footnote>
        ...
        <StyledCard>
          <CardContent>
            ...
            <CustomValue>{totalSupply ? getBalanceNumber(totalSupply.minus(stakedBalance).minus(fishFeedBalance)).toFixed(5) : 'Locked'}</CustomValue>
            <Footnote>
